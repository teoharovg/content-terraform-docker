node {
    ...
    git 'https://github.com/teoharovg/content-terraform-docker.git'
    if (action =='Deploy') {
        stage('init') {
              sh """terraform init"""
        }
    ...
    if(action == "Destroy"){
        ...
        stage('destroy'){
            ...
            sh label:"terraform plan -destroy -out=tfdestroyplan -input=false -var image_name=${image_name} -var ext_port=${ext_port}"
        }
    }
}
